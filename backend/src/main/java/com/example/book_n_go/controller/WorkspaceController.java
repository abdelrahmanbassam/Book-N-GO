...
public class WorkspaceController {
  ...
  public ResponseEntity<Workspace> updateWorkspace(@PathVariable("id") long id, @RequestBody Workspace workspace) {
    ...
    if (!AuthService.getRequestUser().getId().equals(workspaceData.get().getProvider().getId())) {
      return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
    }
  ...
  public ResponseEntity<HttpStatus> deleteWorkspace(@PathVariable("id") long id) {
    ...
    if (!AuthService.userHasPermission(Permission.PROVIDER_DELETE)
        || !AuthService.getRequestUser().getId().equals(workspaceRepo.findById(id).get().getProvider().getId())) {
